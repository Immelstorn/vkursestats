        ...
        public IHttpActionResult Get()
        {
            using (var db = new DataContext())
            {
                ...
                var result = new ReturnDto {
                    Eur = new RateDto {
                        Buy = last[0].EurBuy,
                        ChangePercentBuy = last.Count == 2 ? (last[0].EurBuy / last[1].EurBuy - 1) * 100 : 0,
                        Sell = last[0].EurSell,
                        ChangePercentSell = last.Count == 2 ? (last[0].EurSell / last[1].EurSell - 1) * 100 : 0,
                    },
                    Usd = new RateDto {
                        Buy = last[0].UsdBuy,
                        ChangePercentBuy = last.Count == 2 ? (last[0].UsdBuy / last[1].UsdBuy - 1) * 100 : 0,

                        Sell = last[0].UsdSell,
                        ChangePercentSell = last.Count == 2 ? (last[0].UsdSell / last[1].UsdSell - 1) * 100 : 0,
